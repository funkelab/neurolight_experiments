...
# Get Fusion snapshots using all available data
...
path_to_data = Path("/nrs/funke/mouselight-v3")
...
data_sources = tuple(
    (
        gp.N5Source(
            filename=str((filename / "fluorescence-near-consensus.n5").absolute()),
            datasets={raw: "volume"},
            array_specs={
                raw: gp.ArraySpec(
        ...
        GraphSource(
            filename=str(
                ...
                # ).absolute()
                Path(
                    "/nrs/funke/mouselight-v2/", filename.name, "skeletonization.obj"
                ).absolute()
            ),
        ...
        GraphSource(
            filename=str(
                ...
                # ).absolute()
                Path(
                    "/nrs/funke/mouselight-v2/", filename.name, "consensus_tracings"
                ).absolute()
            ),
    ...
    + TopologicalMatcher(
        skeletonization, consensus, matched, failures=Path("matching_failures_slow")
    )
    + RasterizeSkeleton(
        points=matched,
        array=labels,
