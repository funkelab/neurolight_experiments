...
class Synthetic2DSource(gp.BatchProvider):
    ...
    def provide(self, request):
        ...
        for i in range(self.n_objects):
            points = np.stack([
                    np.random.randint(0, shape[dim], 2)
                    for dim in range(3)
                ], axis=1)
            tree = skelerator.Tree(points)
            skeleton = skelerator.Skeleton(tree, [1, 1, 1], "linear", generate_graph=False)
        ...
        raw_data = np.clip(
            raw_data + np.random.normal(scale=0.1, size=raw_data.shape),
        ...
        spec.voxel_size = (1, 1)
        gt_crop = (request[self.gt].roi - request[self.raw].roi.get_begin())/voxel_size
...
def add_loss(graph):
    ...
    embedding = tf.reshape(
        embedding,
        (1,) + tuple(embedding.get_shape().as_list()))
    # k, 1, h, w
    ...
    fg_loss = tf.losses.mean_squared_error(
        gt_fg,
        fg)
